...
def dashboard(request):
    ...
    # 1. Lógica de Progreso Semanal (Barras L-V)
    ...
    # 2. Último Entrenamiento Real (Busca el día más reciente con éxito)
    ...
    if ultima_fecha:
        ...
        porcentaje_u = int((completados_u / total_u) * 100) if total_u > 0 else 0
        ultimo_entrenamiento_info = {
            'fecha': ultima_fecha,
            'porcentaje': porcentaje_u
        }

    # 3. Mensaje motivador basado en asistencias de los últimos 7 días
    ...
    context = {
        ...
        'ultimo_entrenamiento': ultimo_entrenamiento_info,
        'mensaje_motivador': f"Llevas {asistencias_semana} días entrenando esta semana. ¡Seguí así!",
    }
...
def mi_rutina(request):
    ...
    # Resetear completado si es un día nuevo
...
def marcar_ejercicio_hecho(request, ejercicio_id):
    if request.method == 'POST':
        try:
            ...
            ejercicio.fecha_asignacion = timezone.now().date() # Vital para el dashboard
            ejercicio.save()
...
def agregar_ejercicio(request, alumno_id):
    if request.method == 'POST':
        ...
        messages.success(request, "Ejercicio agregado.")
...
def alta_socio_rapida(request):
    ...
    if request.method == "POST":
        ...
        Alumno.objects.create(
            ...
            plan_semanal=plan, dni=request.POST.get('dni'), activo=True, 
...
def historial_asistencias(request, alumno_id):
    alumno = get_object_or_404(Alumno, id=alumno_id)
    hoy = timezone.now().date()
    asistencias = Asistencia.objects.filter(alumno=alumno).order_by('-fecha')[:30]
    return render(request, 'alumnos/historial_asistencias.html', {'alumno': alumno, 'asistencias': asistencias})
